...
Event.register(defines.events.on_research_finished, function(event)
    ...
    elseif tech_name == 'surveillance-2' then
        if storage.power_poles and storage.surveillance_centers then
            update_all_surveillance(force)
        end
    end
end)
...
Event.register(defines.events.on_sector_scanned, function(event)
    if not storage.following then return end
    if event.radar.name == 'big_brother-surveillance-center' then
        storage.following = table.each(table.filter(storage.following, Game.VALID_FILTER), function(entity)
...
function update_all_surveillance(force)
    table.each(table.filter(storage.vehicles or {}, Game.VALID_FILTER), function(vehicle) update_surveillance(vehicle, true) end)
    table.each(table.filter(storage.trains or {}, Game.VALID_FILTER), function(train) update_surveillance(train, true) end)

    if storage.power_poles and force.technologies['surveillance-2'].researched then
        storage.power_poles = table.each(table.filter(storage.power_poles, Game.VALID_FILTER), function(entity)
            update_surveillance(entity, false)
        end)
    end
end
...
function upgrade_radars(force)
    if not storage.radars then return end
    ...
    local radar_name = 'big_brother-radar_ra-' .. radar_amplifier_level .. '_re-' .. radar_efficiency_level
    for i = #storage.radars, 1, -1 do
        local radar = storage.radars[i]
        if not radar.valid then
            table.remove(storage.radars, i)
        elseif radar.force == force then
            storage.radars[i] = upgrade_radar_entity(radar, radar_efficiency_level, radar_amplifier_level)
        end
    end
end
...
function remove_surveillance(entity, follow)
    if follow then
        storage.following = table.filter(storage.following or {}, function(followed) return followed ~= entity end)
    else
        local data = Entity.get_data(entity)
...
function get_nearest_surveillance_center(position, surface, force)
    if storage.surveillance_centers then
        storage.surveillance_centers = table.filter(storage.surveillance_centers, Game.VALID_FILTER)
        local list = table.filter(storage.surveillance_centers, function(entity)
...
function track_entity(category, entity)
    ...
    if not storage[category] then storage[category] = {} end
    local entity_list = storage[category]
